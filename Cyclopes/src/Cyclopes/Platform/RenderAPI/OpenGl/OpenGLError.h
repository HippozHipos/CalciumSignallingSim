...
namespace cyc {
    ...
    #else
        #define CYC_OPENGL_ERROR_CALLBACK(msg)
    #endif
#endif

#if defined(CYC_DEBUG) || defined(CYC_RELEASE)
    #define CYC_CHECK_SHADER_COMPILATION(shader) CheckShaderCompilationSuccess(shader)
#else
    #define CYC_CHECK_SHADER_COMPILATION(shader)
#endif
