...
#include "Cyclopes/Platform/OS/Windows/WindowsError.h"
#include "wglext.h"
...
namespace cyc {
	...
	void OpenGLContext::Init(Window* window)
	{
		...
		//TODO: WRITE THE OUTPUT OF THEESE ASSERTS AND ERROR CHECKS TO SOME ERROR LOG FILE SINCE THEY ARENT 
		//DISPLAYED IN CONSOLE

		m_Dc = ::GetDC((HWND)window->GetNativeWindow());
		CYC_ASSERT(m_Dc,
			"GetDC() failed. Could not retrieve Device Context from window handle / Invalid window handle");
		...
		//CYC_OPENGL_ERROR_CALLBACK_MSG("Cyclopes Core: Open GL Error");
		//SetOpenGlErrorMsgCallback("");
		...
		MakeCurrent();
		SetSwapInterval(1);
		CYC_OPENGL_ERROR_CALLBACK("Cyclopes OpengGL Error");
	...
	void OpenGLContext::SwapBuffers()
	{
		...
		if (!dc)
		{
			CYC_WARN("[OpenGLContext::SwapBuffers] Couldn't retrieve Device Context from window handle. Is window handle valid?");
			return;
		}
#endif
