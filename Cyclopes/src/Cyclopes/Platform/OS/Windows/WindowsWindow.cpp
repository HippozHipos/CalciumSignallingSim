...
#include "Cyclopes/imgui/backend/imgui_impl_win32.h"
...
namespace cyc {
	...
	namespace {
		...
		bool Cyc_N_RegisterMouseRawInput()
		{
			RAWINPUTDEVICE rid;

			rid.usUsagePage = 0x01;          // HID_USAGE_PAGE_GENERIC
			rid.usUsage = 0x02;              // HID_USAGE_GENERIC_MOUSE
			rid.dwFlags = 0;  //TODO: Setting this to RIDEV_NOLEGACY makes window unresponsive. Investigate the bug.
			rid.hwndTarget = nullptr;

			if (RegisterRawInputDevices(&rid, 1, sizeof(rid)) == FALSE)
			{
				CYC_WIN32_LASTERROR_NOBREAK(false, "RegisterRawInputDevices failed");
				return false;
			}
		...
		RAWINPUT* Cyc_N_GetRawInputData(LPARAM lParam)
		{
			UINT dwSize = 0;
			...
			if (GetRawInputData((HRAWINPUT)lParam, RID_INPUT, lpb, &dwSize, sizeof(RAWINPUTHEADER)) != dwSize)
				CYC_WARN("GetRawInputData did not return correct size\n");
	...
	void Win32NativeWindow::Init(const WindowProperties& props)
	{
		...
		CycWString wTitle = converter.from_bytes(props.title);
	...
	LRESULT Win32NativeWindow::HandleMessage(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		switch (msg)
		{
			case WM_INPUT:
			{
				RAWINPUT* raw = Cyc_N_GetRawInputData(lParam);
			...
			//TODO: UPDATE WINDOWS SIZE STORED IN CLASS SO THAT IT IS ACCURATE WHEN MOUSE EVENTS
			case WM_SIZE:
			{
	...
	void Win32NativeWindow::ErrorMessageBox(CycWString error)
	{
	...
	{
		std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
		CycWString wTitle = converter.from_bytes(title);
